...
export default function TensionsPage() {
  const { tensions, acknowledgeTension } = useAegisStore();
  ...
  const counts = {
    ...
    unresolved: tensions.filter(t => t.status === "UNRESOLVED").length,
    controlled: tensions.filter(t => t.status === "CONTROL_APPLIED").length,
  };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {tensions.map(tension => (
            <TensionCard
              ...
              onAcknowledge={() => acknowledgeTension(tension.id)}
            />
          ))}
        </div>
      </div>
...
function TensionCard({ tension, expanded, onToggle, onAcknowledge }: {
  tension: PolicyTension; expanded: boolean; onToggle: () => void; onAcknowledge: () => void;
}) {
  const sc = SEVERITY_CONFIG[tension.severity] || SEVERITY_CONFIG.MEDIUM;
  const stc = STATUS_CONFIG[tension.status];

  return (
    <div style={{
      background: "#FFFFFF", borderRadius: "14px", overflow: "hidden",
      border: `1px solid ${tension.status === "UNRESOLVED" ? sc.border : "#E5E7EB"}`,
      boxShadow: tension.status === "UNRESOLVED" ? `0 2px 12px ${sc.text}18` : "0 2px 8px rgba(0,0,0,0.04)",
    }}>
      {/* Header */}
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid #F3F4F6", padding: "1.25rem 1.5rem" }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.75rem" }}>
            ...
            {tension.status !== "CONTROL_APPLIED" && (
              <button style={{
                display: "flex", alignItems: "center", gap: "0.5rem",
